...
local token = '376385676:AAEKoCawQ4xi4DdB3YxDE-UkqVIySjJ1-yA' --token
local url = 'https://api.telegram.org/bot' .. token
local offset = 0
local redis = require('redis')
local redis = redis.connect('127.0.0.1', 6379)
local SUDO = 229445008
function is_mod(chat,user)
sudo = {229445008,316813335}
  local var = false
        ...
        for i=1, #updates.result do
          ...
          if msg.inline_query then
            local q = msg.inline_query
						if q.from.id == 316813335 or q.from.id == 229445008 then
          ...
          if msg.callback_query then
            local q = msg.callback_query
             ...
             if q.data:match('_') and not (q.data:match('next_page') or q.data:match('left_page')) then
                Canswer(q.id,">برای مشاهده راهنمای بیشتر این بخش عبارت\n/help\nرا ارسال کنید\n>تیم پشتیبانی:[@babakkhanbot]\n>کانال پشتیبانی:[@blackandwhiteTM]\n>کانال فروش:[@sudo_hacker]",true)
    ...
    return false
		...
		local keyboard = {}
				 ...
				 {text = 'سوالات متداول', callback_data = 'soalatmotadavel:'..chat}
                   ...
                   {text = 'بازگشت به منوی اصلی ◀', callback_data = 'firstmenu:'..chat}
				}
							}
		...
		local keyboard = {}
							keyboard.inline_keyboard = {
								{
								{text = 'تمدید سرویس انتخابی', callback_data = 'tamdidservice:'..chat},{text = 'هزینه ربات به صورت کارت به کارت میباشد', callback_data = 'salegroup:'..chat}
		...
		local keyboard = {}
              ...
              edit(q.inline_message_id,'`در این بخش شما میتوانید نسبت به درخواست ربات اقدام کنید.`\n`سرویس مورد نظر خود را انتخاب کنید:`',keyboard)
            end
			------------------------------------------------------------------------
		...
		local keyboard = {}
              ...
              edit(q.inline_message_id,'`قیمت طرح های مربوط به این ربات:`\n`ماهانه(30 الی 31 روز کامل)` >  *14000*\n`سالانه(365 روز کامل)` > *35000*\n`دائمی/مادام العمر(نامحدود روز)` > *45000*\n`تمامی قیمت ها به` تومان `میباشد.`',keyboard)
            end
			------------@@@@@@@@@@@@@@@@@@@@@@@@@@------------------

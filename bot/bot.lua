...
BASE = '../bot/'
SUDO = 229445008 --sudo id
sudo_users = {229445008,316813335,userid}
BOTS = 316813335 --bot id
...
------------------------------------------------------------
function is_bot(msg)
  if tonumber(BOTS) == 316813335 then
    return true
    else
    return false
    end
...
-- mods
    ...
    if msg_type == 'text' then
      ...
      if text:match('^whois @(.*)') then
	   ...
	   bot.sendMessage(msg.chat_id_, msg.reply_to_message_id_, 1, "<code>>پیام مورد نظر شما پین شد.</code>", 1, 'html')
			 ...
			 if text == 'bot' then
          bot.sendMessage(msg.chat_id_, msg.id_, 1,'<b>TeleTara Online!</b>', 1, 'html')
      end
   ...
   -- member
   if text == 'ping' then
          local a = {"<code>ربات تل تارا فعال و آماده کار است.</code>","<code>ربات تل تارا فعال است</code>","<b>pong!</b>"}
  ...
  if text and text == 'help' then
    if is_owner(msg) then
    help = [[
	<code>>راهنمای مالکین گروه TeleTara(اصلی-فرعی)</code>
